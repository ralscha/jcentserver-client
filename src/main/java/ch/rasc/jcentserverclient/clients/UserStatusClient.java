...
public interface UserStatusClient {
	...
	default UpdateUserStatusResponse updateUserStatus(
			Function<UpdateUserStatusRequest.Builder, UpdateUserStatusRequest.Builder> fn) {
		return this.updateUserStatus(fn.apply(UpdateUserStatusRequest.builder()).build());
	}

	@RequestLine("POST /get_user_status")
	GetUserStatusResponse getUserStatus(GetUserStatusRequest request);

	default GetUserStatusResponse getUserStatus(
			Function<GetUserStatusRequest.Builder, GetUserStatusRequest.Builder> fn) {
		return this.getUserStatus(fn.apply(GetUserStatusRequest.builder()).build());
	}

	@RequestLine("POST /delete_user_status")
	DeleteUserStatusResponse deleteUserStatus(DeleteUserStatusRequest request);

	default DeleteUserStatusResponse deleteUserStatus(
			Function<DeleteUserStatusRequest.Builder, DeleteUserStatusRequest.Builder> fn) {
		return this.deleteUserStatus(fn.apply(DeleteUserStatusRequest.builder()).build());
	}
